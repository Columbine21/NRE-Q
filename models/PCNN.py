...
class PCNN(BasicModel):
    ...
    def init_word_emb(self):
        ...
        w2v = torch.from_numpy(np.load(self.opt.w2d_path))
    ...
    def forward(self, input):
        ...
        assert lexical_level_emb.shape == torch.tensor([batch_size, 6, self.opt.word_dim]), \
            "Lexical Feature shape incorrect."
        lexical_level_emb.view(batch_size, -1)

        word_emb = self.word_embedding(word_features)
        assert word_emb.shape == torch.tensor([batch_size, 100, self.opt.word_dim])
        pos1_emb = self.pos1_embedding(left_pf)
        assert pos1_emb.shape == torch.tensor([batch_size, 100, self.opt.pos_dim])
        pos2_emb = self.pos2_embedding(right_pf)
        assert pos2_emb.shape == torch.tensor([batch_size, 100, self.opt.pos_dim])

        sentence_feature = torch.cat([word_emb, pos1_emb, pos2_emb], dim=2)
        assert sentence_feature.shape == torch.tensor([batch_size, 100, self.opt.word_dim+2*self.opt.pos_dim])
        ...
        sentence_feature = [F.max_pool1d(sentence_feature_tmp, sentence_feature_tmp.size(2)).squeeze(2) for sentence_feature_tmp in sentence_feature]

        sentence_feature = torch.cat(sentence_feature)
