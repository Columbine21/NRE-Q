...
class SEMLoad(object):
    ...
    def save(self):
        if self.train:
            ...
            prefix = 'test'
        np.save(os.path.join(prefix, 'word_feature.npy'), self.word_feautre)
        np.save(os.path.join(prefix, 'left_pf.npy'), self.left_pf)
        np.save(os.path.join(prefix, 'right_pf.npy'), self.right_pf)
        np.save(os.path.join(prefix, 'lexical_feature.npy'), self.lexical_feature)
        np.save(os.path.join(prefix, 'labels.npy'), self.labels)
        np.save(os.path.join(prefix, 'w2v.npy'), self.w2v)
        print('save finish!')
    ...
    # def contruct_sentence_feature(self, sens):
    #     '''
    #     : word embedding
    #     : postion embedding
    #     return:
